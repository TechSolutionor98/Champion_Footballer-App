...
class MatchesScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Auto-select first league if none selected OR if selected league no longer exists
    userAsync.whenData((user) {
      ...
      } else if (selectedLeague != null && leagues.isNotEmpty) {
        // Check if selected league still exists in user's leagues
        final leagueStillExists = leagues.any((l) => l.id == selectedLeague.id);
        
        if (!leagueStillExists) {
          // Selected league was deleted/left, select first available league
          WidgetsBinding.instance.addPostFrameCallback((_) {
            try {
              ref.read(selectedLeagueProvider.notifier).state = leagues.first;
            } catch (_) {}
          ...
          final matchingLeague = leagues.firstWhere(
            (l) => l.id == selectedLeague.id,
            orElse: () => selectedLeague,
          );
          
          if (matchingLeague != selectedLeague && 
              matchingLeague.matches?.length != selectedLeague.matches?.length) {
            WidgetsBinding.instance.addPostFrameCallback((_) {
              try {
                ref.read(selectedLeagueProvider.notifier).state = matchingLeague;
              } catch (_) {}
            });
          }
        }
      } else if (selectedLeague != null && leagues.isEmpty) {
        // User left/deleted all leagues
        WidgetsBinding.instance.addPostFrameCallback((_) {
          try {
    ...
    // Auto-select latest match if none selected
    WidgetsBinding.instance.addPostFrameCallback((_) {
      try {
        ...
        if (leagueNow != null) {
          ...
          if (matches.isNotEmpty) {
            if (currentMatch == null || !matches.any((m) => m.id == currentMatch.id)) {
              ...
              Consumer(builder: (context, ref, _) {
                ...
                if (matches.isEmpty) {
                  return Padding(
                    ...
                    child: Text(
                      "No matches available for ${league.name}",
